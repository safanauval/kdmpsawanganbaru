<x-layouts::app :title="__('Tambah Stok Barang')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl p-6">
        <div class="mb-4">
            <flux:heading size="xl">Tambah Stok Barang
            </flux:heading>
            <p class="mt-3 text-gray-600 dark:text-gray-400">Masukkan data barang baru</p>
        </div>
                ...
                <div class="space-y-4">
                    <div>
                        <div>
                            ...
                            <flux:input class="p-1" type="file" name="gambar" accept="image/jpg/png/jpeg" required>
                            </flux:input>
                            <flux:error name="gambar" />
                        </div>
                        <div>
                            <flux:label class="p-2">Kode Barang</flux:label>
                            <flux:input class="p-1" name="kode_barang" value="{{ old('kode_barang') }}" required>
                            </flux:input>
                            <flux:error name="kode_barang" />
                        </div>
                        <div>
                            <flux:label class="p-2">Nama Barang</flux:label>
                            <flux:input class="p-1" name="nama_barang" value="{{ old('nama_barang') }}" required>
                            </flux:input>
                        ...
                        <div>
                            <flux:label class="p-2">Gudang</flux:label>
                            <flux:select class="p-1" name="gudang_id" required>
                                <option value="">- Pilih Gudang </option>
                                @foreach($gudang as $gd)
                                    <option value="{{ $gd->id }}" {{ old('gudang_id') == $gd->id ? 'selected' : '' }}>
                                        {{ $gd->nama_gudang }}
                                    </option>
                                @endforeach
                            </flux:select>
                            <flux:error name="gudang_id" />
                        </div>
                        <div>
                            <flux:label class="p-2">Stok Awal</flux:label>
                            <flux:input class="p-1" type="number" name="stok" value="{{ old('stok') }}" min="1"
                                required></flux:input>
                            <flux:error name="stok" />
                        </div>
                        <div>
                            <flux:label class="p-2">Satuan</flux:label>
                            <flux:input class="p-1" type="text" name="satuan" value="{{ old('satuan') }}" required>
                            </flux:input>
                            <flux:error name="satuan" />
                        </div>
                        <div>
                            <flux:label class="p-2">Harga Beli (Rp)</flux:label>
                            <flux:input class="p-1" type="number" name="harga_beli" value="{{ old('harga_beli') }}"
                        ...
                        <div>
                            ...
                            <flux:input class="p-1" type="number" name="harga_jual" value="{{ old('harga_jual') }}"
                                min="0" step="0.01" required>
                            </flux:input>
                        ...
                        <div>
                            ...
                            <flux:textarea class="p-1" name="deskripsi" rows="3">{{ old('deskripsi') }}</flux:textarea>
