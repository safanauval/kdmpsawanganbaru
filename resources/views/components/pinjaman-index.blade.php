<div x-data x-on:notify.window="Flux.toast({text: $event.detail[0], variant: $event.detail[1] ?? 'success' })"
                ...
                <flux:table.rows>
                    @forelse($pinjamans as $p)
                        <flux:table.row wire:key="pinjaman-{{ $p->id }}">
                            ...
                            <flux:table.cell align="center"
                                class="font-bold text-base {{ $p->sisa_pinjaman > 0 ? 'text-red-600' : 'text-green-600' }}">
                                Rp {{ number_format($p->sisa_pinjaman, 0, ',', '.') }}
                            </flux:table.cell>
                            <flux:table.cell align="center">
                                <flux:badge
                                    color="{{ $p->status === 'aktif' ? 'blue' : ($p->status === 'lunas' ? 'green' : 'red') }}">
                                    {{ ucfirst($p->status) }}
                                </flux:badge>
                            </flux:table.cell>
                            ...
                            <flux:table.cell align="end" class="py-2">
                                <flux:button.group>
                                    @if($p->status === 'aktif')
                                        <flux:button wire:click="openBayar({{ $p->id }})" size="sm" icon="banknotes"
                                            variant="primary" color="blue">
                                            Bayar
                                        </flux:button>
                                    @endif
                                    <flux:button wire:click="openEdit({{ $p->id }})" size="sm" icon="pencil-square" />
                                    <flux:button wire:click="delete({{ $p->id }})" wire:confirm="Yakin hapus?" size="sm"
    ...
    {{-- Modal Form Pinjaman --}}
    <flux:modal wire:model="showModal" :title="$editMode ? 'Edit Pinjaman' : 'Tambah Pinjaman'" class="max-w-lg"
        style="width: 800px;">
    ...
    <flux:modal wire:model="showBayarModal" title="Bayar Angsuran" class="max-w-lg" style="width: 800px;">
        <div class="space-y-4 p-6">
            ...
            @if($selectedPinjaman)
                <div class="bg-blue-50 dark:bg-blue-900/20 p-4 rounded space-y-2">
                    <div class="flex justify-between text-sm">
                        ...
                        <span
                            class="font-bold text-white-600 dark:text-white-300">{{ $selectedPinjaman->kode_pinjaman }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-white-700 dark:text-white-300">Anggota</span>
                        <span
                            class="font-bold text-white-600 dark:text-white-300">{{ $selectedPinjaman->anggota->nama_anggota ?? '-' }}</span>
                    </div>
                    <div class="border-t border-white-200 dark:border-white-700">
                        <div class="flex justify-between text-sm" style="padding-top: 8px;">
                            ...
                            <span class="font-bold text-red-600">Rp
                                {{ number_format($selectedPinjaman->sisa_pinjaman, 0, ',', '.') }}</span>
                        </div>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-white-700 dark:text-white-300">Angsuran per Bulan</span>
                        <span class="font-bold text-white-600 dark:text-white-300">Rp
                            {{ number_format($selectedPinjaman->angsuran_per_bulan, 0, ',', '.') }}</span>
                    </div>
                    <div class="flex justify-between text-sm">
                        <span class="text-white-700 dark:text-white-300">Angsuran ke</span>
                        <span
                            class="font-bold text-white-600 dark:text-blue-300">{{ $selectedPinjaman->angsurans->count() + 1 }}
                            dari {{ $selectedPinjaman->tenor }}</span>
                    </div>
                </div>
            @endif
            ...
            <flux:field>
                ...
                <flux:input type="number" wire:model="jumlah_bayar" min="0" step="1000" readonly
            ...
            {{-- Tombol Aksi --}}
            <div class="flex gap-2 justify-between pt-4 border-t border-zinc-200 dark:border-zinc-700">
                <flux:button wire:click="$set('showBayarModal', false)" color="red" variant="danger"
                    style="width: 50%;">
                    Batal
                </flux:button>
                <flux:button wire:click="bayarAngsuran" variant="primary" color="blue" style="width: 50%;"
                    icon="banknotes">
                    Bayar Angsuran
                </flux:button>
            </div>
