                ...
                @forelse($stokBarang as $item)
                    <flux:table.row wire:key="item-{{ $item->id }}">
                        <flux:table.cell class="py-2">
                            <div class="w-10 h-10 rounded-lg bg-neutral-100 dark:bg-neutral-800 flex items-center justify-center overflow-hidden">
                                @if($item->gambar_url)
                                    <img src="{{ $item->gambar_url }}" alt="{{ $item->nama_barang }}" class="w-full h-full object-cover">
                                @else
                        ...
                        <flux:table.cell class="text-neutral-600 dark:text-neutral-400">{{ $item->kategori->nama ?? '-' }}</flux:table.cell>
                        <flux:table.cell class="text-neutral-600 dark:text-neutral-400">{{ $item->gudang->nama_gudang ?? '-' }}</flux:table.cell>
                        ...
                        <flux:table.cell align="end">
                            <div class="flex justify-end gap-2">
                                <flux:button.group>
                                    ...
                                    <flux:button wire:click="delete({{ $item->id }})" wire:confirm="Yakin hapus barang ini?" size="sm" icon="trash" variant="danger">
                                        Hapus
                                    </flux:button>
                                </flux:button.group>
                            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <flux:label>Satuan</flux:label>
                    <flux:input 
                        ...
                        wire:model="satuan" 
                        list="satuan-list"
                        placeholder="Masukkan satuan (pcs, lusin, dus, kodi, dll)"
                        required 
                    />
